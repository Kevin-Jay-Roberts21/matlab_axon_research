...
P1 = 0.0050; % ending position of adding the stimulus (in cm or *10^4 in um)
...
for j = 1:(n-1)
    ...
    for i = 1:m
        ...
        a1 = -a(i)/(2*r_l*h^2);
        a2 = a(i)/(r_l*h^2) + c_m(i)/k + g_k(i)*N(1, i)^4 + g_Na(i)*M(1, i)^3*H(1, i) + g_L;
        a3 = -a(i)/(2*r_l*h^2); 
        a4 = c_m(i)/k; 
        ...
        % if j*k >= T0 && j*k <= T1
        %     a2 = a(i)/(r_l*h^2) + c_m(i)/k + g_k(i)*N(1, i)^4 + (g_Na(i)*M(1, i)^3*H(1, i) + S) + g_L;
