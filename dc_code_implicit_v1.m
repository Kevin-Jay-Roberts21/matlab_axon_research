...
V_my0 = 0.0368; % (mV) initial condition for axon potential in periaxonal space
...
for j = 1:(n-1)
    ...
    for i = 2:(m-1)
        ...
        if (i > myelin_start + 1) && (i < myelin_end + 1) % Internodal region
            newVmy(i) = rho*w1/2*Vm(i-1) - rho*w1*Vm(i) + rho*w1/2*Vm(i+1) + rho*w2/2*Vmy(i-1) + (1 - rho*w2 - 1/(R_my*C_my))*Vmy(i) + rho*w2/2*Vmy(i+1); 
        elseif (i > seg_start + 1) && (i < myelin_start + 1) % Nodal region
            newVmy(i) = 0;
        elseif (i == seg_start + 1) % Right end point (x_R)
            newVmy(i) = rho*w1/2*Vm(i-1) - rho*w1*Vm(i) + rho*w1/2*Vm(i+1) + rho*w2/2*(1 + w3*dx)*Vmy(i) + (1 - rho*w2 - 1/(R_my*C_my))*Vmy(i) + rho*w2/2*Vmy(i+1); 
        elseif (i == myelin_start + 1) % Left end point (x_L)
