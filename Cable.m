...
for j = 1:(n-1)
    ...
    for i = 1:m
        ...
        % adding a stimulus of 0.(from 5ms to 8ms)
%         if j > 5/0.005 && j < 8/0.005
%             a2 = a/(r_l*h^2) + c_m/k + g_k*N(i, 1)^4 + (g_Na*M(i, 1)^3*H(i, 1) + 0.12) + g_L;
%             a5 = g_k*N(i, 1)^4*E_k + (g_Na*M(i, 1)^3*H(i, 1) + 0.12)*E_Na + g_L*E_L;
%         end
