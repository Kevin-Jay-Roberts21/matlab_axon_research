...
% m = L/dx + 1; % total number of space steps
n = T/dt + 1; % total number of time steps
sigma_1 = 1500; % the standard deviation of noise (uA/cm^2, microampere per square centimeter) 
...
for i = 1:(n-1)
    ...
    N_3(i+1) = (1 + dt*(alpha_n(Vm(i)) + 3*beta_n(Vm(i))))^(-1)*(N_3(i) + dt*(4*beta_n(Vm(i))*N_4(i) + 2*alpha_n(Vm(i))*N_3(i)));
    ...
    % Changing the ion channel coefficients
    % N_coef = 
    % M_coef = 
    % H_coef = 


    Vm(i+1) = (Vm(i) + dt/C_m*xi_1 + dt/C_m*G_Na**E_Na + dt/C_m*G_K*(N_4(i+1))^4*E_K + dt/C_m*G_L*E_L)/(1 + dt/C_m*G_Na*(M_3(i+1))^3*H_1(i+1) + dt/C_m*G_K*(N_4(i+1))^4 + dt/C_m*G_L);
